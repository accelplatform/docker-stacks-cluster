...
- intra-mart Accel Platform Professional Edition 2026 Spring
  ...
  - AccelStudio 8.0.9
- Oracle Database Free 26ai（23.26.1.0）

※ Oracle Database Free は開発・テスト用途に限定され、本番サーバーとしての使用はできません。
...
```
 ...
 └───┬───┘   └───┬───┘                └──────────────────┘
  ...
  ／   │   ＼   ／   ＼  ＼
┌──────┐ ┌──────┐ ┌─────────┐ ┌───────┐ ┌─────────────────────────┐
│oracle│ │ solr │ │cassandra│ │mailpit│ │accelstudio-testing-agent│
└──────┘ └──────┘ └─────────┘ └───────┘ └─────────────────────────┘
```
...
Ubutun＋Docker環境では以下のようなエラーが出て正常に起動できない場合があります。

```sh
# コンテナを起動（フォアグラウンド・モード）
docker compose up
...
oracle-1     | Enter SYS user password:
oracle-1     |*********
oracle-1     | Enter SYSTEM user password:
oracle-1     | ********
oracle-1     | Enter PDBADMIN User Password:
oracle-1     | *******
oracle-1     | Prepare for db operation
oracle-1     | Cannot create directory "/opt/oracle/oradata/FREE".
oracle-1     | 7% complete
oracle-1     | 100% complete
oracle-1     | [FATAL] Prepare Operation has failed.
oracle-1     |    CAUSE: No additional information available.
oracle-1     |    ACTION: Refer to the logs or contact Oracle Support Services
oracle-1     | 0% complete
oracle-1     | Look at the log file "/opt/oracle/cfgtoollogs/dbca/FREE/FREE.log" for further details.
oracle-1     |
oracle-1     | Database configuration failed. Check logs under '/opt/oracle/cfgtoollogs/dbca'.
```

コンテナを停止し、 `/data/oracle` フォルダに権限を付与してから再度コンテナを起動してください。
...
# docker compose down accelstudio-testing-agent
# データの削除
Remove-Item -Recurse -Force data/cassandra, data/httpd, data/mailpit, data/oracle, data/resin, data/resin1, data/resin2, data/solr, data/accelstudio-testing-agent
# コンテナの起動
docker compose up -d
```
